...
async def chat(req: dict):
    ...
    prompt = (
        ...
        f"用户当前需求：{message}\n\n"
        f"检索到的匹配商品：\n{products_text}\n\n"
        "请从匹配商品中挑2-3个最合适的推荐给用户。"
        "每个推荐必须包含：① 商品名和价格 ② 穿起来/用起来/吃起来的具体效果和感受 ③ 为什么适合用户。"
        "要像闺蜜/哥们分享好物一样自然，有画面感。控制在250字以内。"
    )
...
async def recommend(file: UploadFile = File(...)):
    ...
    try:
        ...
        prompt = (
            ...
            "以下是检索到的相似商品列表：\n"
            f"{products_text}\n\n"
            "请根据视觉特征和检索结果推荐2-3个最合适的商品。"
            "说明上身效果/使用感受，像闺蜜分享好物一样有画面感。控制在 200 字以内。"
        )
